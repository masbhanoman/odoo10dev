...
class UserAccessMatrixXlsx(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        print("obj = ",obj, "obj.field",obj.user_groups)
        ...
        # # # installed app list from user_groups data in column
        ...
        global_pos_start = 0
        global_pos_end = 0
        rg = self.env['res.groups'].search([('id','=',obj.user_groups.id)])
        #rg = self.env['res.groups'].browse(obj.user_groups)
        rg = rg.sorted(key='display_name')
        for col_pos, group in enumerate(rg):
...
UserAccessMatrixXlsx('report.gbs_user_access_matrix_report.report_uam_name.xlsx',
            'select.uam')